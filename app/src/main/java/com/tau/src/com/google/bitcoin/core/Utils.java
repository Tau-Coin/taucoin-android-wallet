...
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.digests.RIPEMD160Digest;
...
public class Utils {
    ...
    /** Returns the given string hex decoded. */
    ...
    public static String getHash160FromTauAddress(String tauAddress) throws AddressFormatException {
        if (tauAddress == null || "".equals(tauAddress)) {
            throw new AddressFormatException("Invalid TAU address");
        }

        byte[] base58Decode = null;
        try {
            base58Decode = Base58.decode(tauAddress);
        } catch (AddressFormatException e) {
            throw e;
        }

        String decodeStr = Utils.bytesToHexString(base58Decode);
        if (decodeStr.length() != 50) {
            throw new AddressFormatException("Invalid TAU address");
        }
        String version = decodeStr.substring(0, 2);
        if (!"41".equals(version)) {
            throw new AddressFormatException("Invalid TAU address");
        }
        String hash160 = decodeStr.substring(2, 42);
