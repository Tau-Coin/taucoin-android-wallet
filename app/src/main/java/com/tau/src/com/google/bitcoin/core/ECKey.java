...
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.ASN1InputStream;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.ASN1OutputStream;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DERBitString;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DERInteger;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DEROctetString;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DERSequence;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DERSequenceGenerator;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.DERTaggedObject;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.sec.SECNamedCurves;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.asn1.x9.X9ECParameters;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.AsymmetricCipherKeyPair;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.generators.ECKeyPairGenerator;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.params.ECDomainParameters;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.params.ECKeyGenerationParameters;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.params.ECPrivateKeyParameters;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.params.ECPublicKeyParameters;
import com.mofei.tau.src.com.google.bitcoin.bouncycastle.crypto.signers.ECDSASigner;
...
public class ECKey implements Serializable {
    ...
    private byte[] compressedPub;
    
    transient private byte[] pubKeyHash;
    transient private byte[] compressedPubKeyHash;
    ...
    public ECKey() {
        ...
        initCompressedPubkey(pubParams.getQ());
    }
    ...
    private void initCompressedPubkey(ECPoint Q) {
        BigInteger x = Q.getX().toBigInteger();
        BigInteger y = Q.getY().toBigInteger();
        byte[] xBytes = x.toByteArray();
        byte[] yBytes = y.toByteArray();
        String xStr = Utils.bytesToHexString(xBytes);
        if (xStr.length() < 64) {
            compressedPub = null;
            return;
        }
        xStr = xStr.substring(xStr.length() - 64, xStr.length());
        if ((int)yBytes[yBytes.length - 1] % 2 == 0) {
            compressedPub = Utils.hexStringToBytes("02" + xStr);
        } else {
            compressedPub = Utils.hexStringToBytes("03" + xStr);
        }
    }
    ...
    public ECKey(BigInteger privKey) {
        ...
        initCompressedPubkey(ecParams.getG().multiply(privKey));
    }
    ...
    public byte[] getCompressedPubKeyHash() {
        if (compressedPubKeyHash == null)
            compressedPubKeyHash = Utils.sha256hash160(this.compressedPub);
        return compressedPubKeyHash;
    }
    ...
    private static byte[] encodeSignatureToDER(BigInteger r, BigInteger s) {
        byte[] der = new byte[70];
        byte[] rBytes = r.toByteArray();
        byte[] sBytes = s.toByteArray();
        byte[] rDER = new byte[32];
        byte[] sDER = new byte[32];

        System.arraycopy(rBytes, rBytes.length - 32, rDER, 0, 32);
        System.arraycopy(sBytes, sBytes.length - 32, sDER, 0, 32);

        System.out.println("DER R hex:" + Utils.bytesToHexString(rDER));
        System.out.println("DER S hex:" + Utils.bytesToHexString(sDER));

        der[0] = (byte)0x30;
        der[1] = (byte)0x44;
        der[2] = (byte)0x02;
        der[3] = (byte)0x20;
        System.arraycopy(rDER, 0, der, 4, 32);
        der[36] = (byte)0x02;
        der[37] = (byte)0x20;;
        System.arraycopy(sDER, 0, der, 38, 32);
    ...
    public byte[] sign(byte[] input) {
        ...
        System.out.println("R hex:" + Utils.bytesToHexString(sigs[0].toByteArray()));
        System.out.println("S hex:" + Utils.bytesToHexString(sigs[1].toByteArray()));
        //byte[] derBytes = encodeSignatureToDER(sigs[0], sigs[1]);
